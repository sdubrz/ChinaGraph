...
def create_pictures(path="", image_shape=(28, 28), inv=False, enlarge=1, trans=False):
    ...
    X0 = np.loadtxt(path+"origin.csv", dtype=np.float, delimiter=",")
    if np.max(X0) < 1.1:
        X = X0 * 255 // 1
    else:
        X = X0 // 1
...
def mnist_scatter(data_name=None, params=None):
    option = 2
    ...
    if option == 1:  # 直接画散点图
        ...
        mnist_images(path, eta=0.3, y_name=params+".csv", image_shape=(128, 128), colormap='gray', inv=False, trans=True)  # 搜 反转
    else:  # 画部分点的艺术散点图
